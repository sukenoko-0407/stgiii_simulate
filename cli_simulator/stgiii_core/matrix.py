...
from .config import SimulationConfig, OperatorType, NonlinearityType
...
def _apply_nonlinearity(
    ...
    """非線形を適用（1回のみ）。"""
    if nonlinearity == NonlinearityType.TANH:
        return np.tanh(x)
    if nonlinearity == NonlinearityType.GELU:
        # Gaussian Error Linear Unit (approx)
...
def _pca_center_only(
    x: NDArray[np.float64],
    n_components: int
) -> tuple[NDArray[np.float64], NDArray[np.float64], NDArray[np.float64]]:
    """
    PCA（center only）を実施し、低次元表現を返す。

    Returns:
        (x_low, components, mean)
    """
    ...
    use = min(n_components, available)
    components = vt[:use]
    x_low = x_centered @ components.T

    if use < n_components:
        # パディングして固定次元を維持
        pad_cols = n_components - use
        x_low = np.pad(x_low, ((0, 0), (0, pad_cols)), mode="constant")
        components = np.pad(components, ((0, pad_cols), (0, 0)), mode="constant")

    return (
        x_low.astype(np.float64, copy=False),
        components.astype(np.float64, copy=False),
        mean.astype(np.float64, copy=False),
    )


_CONTINUOUS_OPERATOR_TYPES = {
...
@dataclass
class OperatorFeatureBundle:
    """Operator向けの特徴量バンドル（BB単位）"""

    bb_high: List[NDArray[np.float64]]  # slot -> (N_s, high_dim)
    bb_low: List[NDArray[np.float64]]   # slot -> (N_s, pca_dim)
    transform_params: Dict[str, NDArray[np.float64]]
    pca_components: List[NDArray[np.float64]]
    pca_means: List[NDArray[np.float64]]
...
class Matrix:
    ...
    # Operator向け特徴量（必要時のみ）
    operator_features: Optional[OperatorFeatureBundle] = None
...
class MatrixGenerator:
    ...
    def _generate_single(self) -> Matrix | None:
        ...
        # Operator向け特徴量（連続相互作用用）
        operator_features: Optional[OperatorFeatureBundle] = None
        if self.config.operator_type in _CONTINUOUS_OPERATOR_TYPES:
            # 生成後にRNG状態を復元して、行列生成の再現性を保つ
            rng_state = self.rng.bit_generator.state
            operator_features = self._generate_operator_features(bb_embeddings)
    ...
    def _generate_operator_features(
        self,
        bb_embeddings: List[NDArray[np.float64]]
    ) -> OperatorFeatureBundle:
        """Operator向けの高次元変換＋PCA特徴量を生成。"""
        d = self.config.embedding_dim
        h = self.config.operator_mlp_hidden_dim
        high_dim = self.config.operator_high_dim
        pca_dim = self.config.operator_pca_dim

        # 固定ランダム変換（試行rngから生成）
        w1 = self.rng.normal(0.0, 1.0, size=(d, h)).astype(np.float64, copy=False)
        b1 = self.rng.normal(0.0, 1.0, size=(h,)).astype(np.float64, copy=False)
        w2 = self.rng.normal(0.0, 1.0, size=(h, high_dim)).astype(np.float64, copy=False)
        b2 = self.rng.normal(0.0, 1.0, size=(high_dim,)).astype(np.float64, copy=False)

        bb_high: List[NDArray[np.float64]] = []
        bb_low: List[NDArray[np.float64]] = []
        pca_components: List[NDArray[np.float64]] = []
        pca_means: List[NDArray[np.float64]] = []

        for z in bb_embeddings:
            hidden = z @ w1 + b1
            v_high = hidden @ w2 + b2
            v_high = _apply_nonlinearity(v_high, self.config.operator_nonlinearity)
            bb_high.append(v_high.astype(np.float64, copy=False))

            v_low, components, mean = _pca_center_only(v_high, pca_dim)
            bb_low.append(v_low)
            pca_components.append(components)
            pca_means.append(mean)

        return OperatorFeatureBundle(
            bb_high=bb_high,
            bb_low=bb_low,
            transform_params={
                "W1": w1,
